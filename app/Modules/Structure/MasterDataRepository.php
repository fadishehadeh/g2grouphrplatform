...
final class MasterDataRepository
{
    ...
    public function findCompany(int $id): ?array
    {
        return $this->database->fetch(
            'SELECT id, name, code, email, phone, address_line_1, address_line_2, city, state, country, postal_code, timezone, status
    ...
    public function updateCompany(int $id, array $data): void
    {
        ...
        $this->database->execute(
            'UPDATE companies SET
                ...
                postal_code = :postal_code, timezone = :timezone, status = :status
             WHERE id = :id',
            $data
        );
    }
    ...
    public function createCompany(array $data): int
    {
        $this->database->execute(
            'INSERT INTO companies (
                name, code, email, phone, address_line_1, address_line_2, city, state, country, postal_code, timezone, status
             ) VALUES (
                :name, :code, :email, :phone, :address_line_1, :address_line_2, :city, :state, :country, :postal_code, :timezone, :status
             )',
            $data
